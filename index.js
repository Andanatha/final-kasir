...
function exportToExcel() {
    ...
    // Siapkan data untuk ditulis ke dalam worksheet
    const data = [
        ["Nama Pembeli", "Rincian Transaksi", "Total", "Kembalian"]
    ];

    riwayatTransaksi.forEach(transaksi => {
        data.push([
            transaksi.nama,
            transaksi.rincian.replace(/<[^>]+>/g, ''), // Menghapus HTML tags dari rincian
            transaksi.total,
            transaksi.kembalian
        ]);
    });
...
// Tambahkan event listener untuk tombol "Ekspor ke Excel"
